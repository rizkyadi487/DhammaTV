...
class data_karyawan extends CI_Controller {
	...
	function edit($id_param)
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$get = $this->db->get_where("dhammadb_karyawan",$id)->row();
	...
	function simpan()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			if($st=="tambah")
			{
				$this->db->insert("dhammadb_karyawan",$dt);
				redirect("dashboard/data_karyawan");
			}
			else if($st=="edit")
			{
				$this->db->update("dhammadb_karyawan",$dt,$id);
				redirect("dashboard/data_karyawan");
			}
		}
		else
	...
	function hapus($id_param)
	{
		...
		{
			$id['id_karyawan'] = $id_param;
			$get = $this->db->delete("dhammadb_karyawan",$id);
			redirect("dashboard/data_karyawan");
		}
		else
		{
