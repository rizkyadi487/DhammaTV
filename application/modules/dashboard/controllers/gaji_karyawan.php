...
class gaji_karyawan extends CI_Controller {
	...
	function tambah()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$d['karyawan'] = $this->db->get("dhammadb_karyawan");
	...
	function edit($id_param)
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$d['mark_pembayaran'] = "";
			$d['mark_jenis_cetakan'] = "";
			$d['mark_jenis_satuan'] = ""; $d['mark_acara'] = "";
			$d['mark_belum_lunas'] = "";
			
			$id['id_gaji_karyawan'] = $id_param;
			$get = $this->db->get_where("dhammadb_gaji_karyawan",$id)->row();
	...
	function simpan()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			if($st=="tambah")
			{
				$this->db->insert("dhammadb_gaji_karyawan",$dt);
				redirect("dashboard/gaji_karyawan");
			}
			else if($st=="edit")
			{
				$this->db->update("dhammadb_gaji_karyawan",$dt,$id);
				redirect("dashboard/gaji_karyawan");
			}
		}
		else
	...
	function hapus($id_param)
	{
		...
		{
			$id['id_gaji_karyawan'] = $id_param;
			$get = $this->db->delete("dhammadb_gaji_karyawan",$id);
			redirect("dashboard/gaji_karyawan");
		}
		else
		{
