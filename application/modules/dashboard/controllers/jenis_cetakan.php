<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');

class jenis_cetakan extends CI_Controller {

	function index($uri=0)
	{
		if($this->session->userdata("logged_in")!="")
		{
			$d['mark_dashboard'] = "";
			$d['mark_pelanggan'] = "";
			$d['mark_user'] = "";
			$d['mark_pemesanan'] = "";
			$d['mark_pembayaran'] = "";
			$d['mark_jenis_cetakan'] = "active";
			$d['mark_jenis_satuan'] = ""; 
			$d['mark_acara'] = "";
			$d['mark_belum_lunas'] = "";
			
			$d['dt_retrieve'] = $this->app_load_data_model->indexs_data_jenis_cetakan($GLOBALS['site_limit_medium'],$uri);
			
 			$this->load->view($GLOBALS['site_theme']."/bg_header");
 			$this->load->view($GLOBALS['site_theme']."/bg_left",$d);
 			$this->load->view($GLOBALS['site_theme']."/jenis_cetakan/bg_home");
 			$this->load->view($GLOBALS['site_theme']."/bg_footer");
		}
		else
		{
			redirect("login");
		}
	}

	function tambah()
	{
		if($this->session->userdata("logged_in")!="")
		{
			$d['mark_dashboard'] = "";
			$d['mark_pelanggan'] = "";
			$d['mark_user'] = "";
			$d['mark_pemesanan'] = "";
			$d['mark_pembayaran'] = "";
			$d['mark_jenis_cetakan'] = "active";
			$d['mark_jenis_satuan'] = ""; 
			$d['mark_acara'] = "";
			$d['mark_belum_lunas'] = "";
			
			$d['satuan'] = $this->db->get("dhammadb_jenis_kerjaan");
			
			$d['id_param'] = "";
			$d['nama_cetakan'] = "";
			$d['harga'] = "";
			$d['id_jenis_satuan'] = "";
			$d['st'] = "tambah";
			
 			$this->load->view($GLOBALS['site_theme']."/bg_header");
 			$this->load->view($GLOBALS['site_theme']."/bg_left",$d);
 			$this->load->view($GLOBALS['site_theme']."/jenis_cetakan/bg_input");
 			$this->load->view($GLOBALS['site_theme']."/bg_footer");
		}
		else
		{
			redirect("login");
		}
	}

	function edit($id_param)
	{
		if($this->session->userdata("logged_in")!="")
		{
			$d['mark_dashboard'] = "";
			$d['mark_pelanggan'] = "";
			$d['mark_user'] = "";
			$d['mark_pemesanan'] = "";
			$d['mark_pembayaran'] = "";
			$d['mark_jenis_cetakan'] = "active";
			$d['mark_jenis_satuan'] = ""; 
			$d['mark_acara'] = "";
			$d['mark_belum_lunas'] = "";
			
			$d['satuan'] = $this->db->get("dhammadb_jenis_kerjaan");
			
			$id['kode_jenis_cetakan'] = $id_param;
			$get = $this->db->get_where("dhammadb_nama_crew",$id)->row();
			$d['id_param'] = $get->kode_jenis_cetakan;
			$d['nama_cetakan'] = $get->nama_cetakan;
			$d['harga'] = $get->harga;
			$d['id_jenis_satuan'] = $get->id_jenis_satuan;
			$d['st'] = "edit";
			
 			$this->load->view($GLOBALS['site_theme']."/bg_header");
 			$this->load->view($GLOBALS['site_theme']."/bg_left",$d);
 			$this->load->view($GLOBALS['site_theme']."/jenis_cetakan/bg_input");
 			$this->load->view($GLOBALS['site_theme']."/bg_footer");
		}
		else
		{
			redirect("login");
		}
	}

	function simpan()
	{
		if($this->session->userdata("logged_in")!="")
		{
			$id['kode_jenis_cetakan'] = $_POST['id_param'];
			$dt['nama_cetakan'] = $_POST['nama_cetakan'];
			$dt['id_jenis_satuan'] = $_POST['id_jenis_satuan'];
			$dt['harga'] = 1;
			$st = $_POST['st'];
			
			if($st=="tambah")
			{
				$this->db->insert("dhammadb_nama_crew",$dt);
				redirect("dashboard/jenis_cetakan");
			}
			else if($st=="edit")
			{
				$this->db->update("dhammadb_nama_crew",$dt,$id);
				redirect("dashboard/jenis_cetakan");
			}
		}
		else
		{
			redirect("login");
		}
	}

	function set()
	{
		if($this->session->userdata("logged_in")!="")
		{
			$set['key'] = $_POST['key'];
			$this->session->set_userdata($set);
			redirect("dashboard/jenis_cetakan");
		}
		else
		{
			redirect("login");
		}
	}

	function hapus($id_param)
	{
		if($this->session->userdata("logged_in")!="")
		{
			$id['kode_jenis_cetakan'] = $id_param;
			$get = $this->db->delete("dhammadb_nama_crew",$id);
			redirect("dashboard/jenis_cetakan");
		}
		else
		{
			redirect("login");
		}
	}
}
